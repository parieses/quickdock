...
TMP="$(mktemp -d)"
trap 'hdiutil detach "/Volumes/QuickDock" >/dev/null 2>&1 || true; rm -rf "$TMP"' EXIT
...
# 卷名可能带版本号，动态找 .app
SRC="$(find "/Volumes" -maxdepth 2 -name '快启坞.app' -o -maxdepth 2 -name 'QuickDock.app' 2>/dev/null | head -1)"
[ -n "$SRC" ] || SRC="$(find "/Volumes" -maxdepth 2 -name '*.app' 2>/dev/null | head -1)"
...
# 5) 卸载 + 清 quarantine
hdiutil detach "$(dirname "$SRC")" >/dev/null 2>&1 || true
echo "==> 清除隔离属性"
