...
func (a *AIService) streamAIChat(ctx context.Context, cfg AIProfile, messages []map[string]interface{}, convID string, onToken func(text string), onReasoning func(text string), onUsage func(promptTokens, completionTokens int)) (string, error) {
...
func (a *AIService) buildAIMessages(ctx context.Context, cfg AIProfile, convID, mode, currentUser string) ([]map[string]interface{}, error) {
	hist, err := a.App.DB.ListAIMessages(convID)
	if err != nil {
		return nil, err
	}
	conv, err := a.App.DB.GetAIConversation(convID)
	if err != nil {
		return nil, err
	}
	...
	if estTokens(hist) > aiTokenBudget && len(hist) > aiKeepRecent {
		...
		if len(toSum) >= aiKeepRecent {
			...
			if summary, serr := a.summarizeAndStore(ctx, cfg, conv, toSum); serr == nil {
				_ = a.App.DB.DeleteOldAIMessages(convID, aiKeepRecent)
				hist = recent
...
func (a *AIService) summarizeAndStore(ctx context.Context, cfg AIProfile, conv *db.AIConversation, toSum []db.AIMessage) (string, error) {
	...
	if err := a.App.DB.UpdateAIConversationMeta(conv.ID, "", combined); err != nil {
		return "", err
	}
	return combined, nil
}
...
func (a *AIService) callAIOnce(ctx context.Context, cfg AIProfile, messages []map[string]string) (string, error) {
