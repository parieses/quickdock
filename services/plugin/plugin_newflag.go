...
func (p *PluginService) loadPluginNewFlags() map[string]bool {
	m := make(map[string]bool)
	if p.App.DB == nil {
		return m
	}
	raw, err := p.App.DB.GetSetting(pluginNewFlagsKey)
	if err != nil || raw == "" {
		return m
	}
...
func (p *PluginService) savePluginNewFlags(m map[string]bool) {
	if p.App.DB == nil {
		return
	}
	...
	_ = p.App.DB.SetSetting(pluginNewFlagsKey, string(b))
}

// MarkPluginNew 内部方法：安装 / 更新插件后由 InstallPlugin 调用，标记该插件为「新」。
// 不导出 —— 只允许后端在写入插件记录后触发，避免被前端误调用。
func (p *PluginService) MarkPluginNew(id string) {
	if id == "" {
		return
	}
	m := p.loadPluginNewFlags()
	if m[id] {
		return // 已是新状态，无需重复写盘
	}
	m[id] = true
	p.savePluginNewFlags(m)
}

// MarkPluginSeen 清除某插件的「新」角标（首次打开后由前端调用）。
func (p *PluginService) MarkPluginSeen(id string) *services.ApiResult {
	if p.App.DB == nil {
	...
	m := p.loadPluginNewFlags()
	if !m[id] {
		return services.Ok(nil) // 本就没有角标，幂等
	}
	delete(m, id)
	p.savePluginNewFlags(m)
	return services.Ok(nil)
...
func (p *PluginService) GetPluginNewFlags() []string {
	if p.App.DB == nil {
		return nil
	}
	m := p.loadPluginNewFlags()
