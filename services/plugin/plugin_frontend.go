...
func (p *PluginService) GetPluginFrontendURL(pluginID string) *services.ApiResult {
	if p.App.PluginMgr == nil {
		return services.FailMsg("plugin manager not initialized")
	}
	path, err := p.App.PluginMgr.GetFrontendPath(pluginID)
...
func (p *PluginService) GetPluginIcon(pluginID string) *services.ApiResult {
	...
	if fi, serr := os.Stat(iconPath); serr == nil && fi.Size() > maxIconSize {
		return services.Fail(fmt.Errorf("图标文件过大 (%d bytes)", fi.Size()))
	}
...
func (p *PluginService) GetPluginFrontendPage(pluginID string, theme string, locale string) *services.ApiResult {
	if p.App.PluginMgr == nil {
		return services.FailMsg("plugin manager not initialized")
	}
	// 「关窗即终止」后插件可能已停止，按需惰性复活再读页面（Show 已确保加载，这里兜底直接调用路径）
	_ = p.App.PluginMgr.EnsureLoaded(pluginID)
	inst := p.App.PluginMgr.GetPlugin(pluginID)
	if inst == nil {
		return services.FailMsg("插件未加载")
	}
	if inst.GetStatus() != "running" {
		return services.FailMsg("插件未运行，无法打开前端页面")
	}
	if !inst.Manifest.Frontend.Enabled {
		return services.FailMsg("插件未启用前端")
	}
	// 防路径穿越：manifest 声明的前端入口必须落在插件目录内
	entryPath, err := safePluginPath(inst.Dir, inst.Manifest.Frontend.Entry)
	if err != nil {
		return services.Fail(fmt.Errorf("前端入口路径非法: %w", err))
	}
	...
	commonCSSPath := filepath.Join(p.App.PluginsDir, "builtin", "common.css")
	...
	// 注入 common.js
	commonJSPath := filepath.Join(p.App.PluginsDir, "builtin", "common.js")
	...
	html = inlineFileRefs(html, baseDir, inlineCSSRe, func(match string) string {
		...
		if strings.HasSuffix(href, "common.css") {
			data, err = os.ReadFile(filepath.Join(p.App.PluginsDir, "builtin", "common.css"))
		} else {
			var p string
	...
	html = inlineFileRefs(html, baseDir, inlineJSRe, func(match string) string {
		...
		if strings.HasSuffix(src, "common.js") {
			data, err = os.ReadFile(filepath.Join(p.App.PluginsDir, "builtin", "common.js"))
		} else {
			var p string
	...
	// 写入缓存
	p.frontendCacheMu.Lock()
	p.frontendCache[pluginID] = &frontendCacheEntry{html: html, htmlMtime: fi.ModTime(), commonMtime: commonMtime}
	p.frontendCacheMu.Unlock()

	return services.Ok(html)
}
