...
// mirrorProvider 对支持 Range 的大包（> dlutil.ParallelMinSize）应走 4 连接分块下载，
...
func TestMirrorProviderDownloadParallel(t *testing.T) {
	...
	if n := rangeReqs.Load(); n != dlutil.DownloadConcurrency {
		t.Fatalf("Range 请求数错误: got %d, want %d（未走并行分块?）", n, dlutil.DownloadConcurrency)
	}
