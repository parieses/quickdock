...
func (s *SQLRuntime) SupportedPlatforms() []string { return []string{"windows", "darwin"} }
...
// serverPath 返回版本目录下的服务二进制；MariaDB 优先 mariadbd、MySQL 优先 mysqld，
// 二者若存在其一即返回（兼容不同构建命名）。mac 下去掉 .exe 后缀。
func (s *SQLRuntime) serverPath(version string) string {
	dir := s.versionDir(version)
	main := strings.TrimSuffix(s.flavor.serverBin, ".exe")
	alt := "mysqld"
	if s.flavor.kind == RuntimeMariaDB {
		alt = "mariadbd"
	}
	cands := []string{main, alt}
	for _, c := range cands {
	...
	return filepath.Join(dir, "bin", main)
}
...
var _ ServiceController   = (*SQLRuntime)(nil) // 服务启停与状态
var _ LogProvider         = (*SQLRuntime)(nil) // 运行日志弹窗
var _ DataDirProvider     = (*SQLRuntime)(nil) // 数据目录定位
var _ ConfigPortsProvider = (*SQLRuntime)(nil) // 配置内端口解析
