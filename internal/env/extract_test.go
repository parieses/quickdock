...
// TestGhInstallLiftsBin 复现 gh 官方 Windows zip 场景：顶层同时含 bin/ 与 LICENSE 等
// 散条目（不满足单一顶层目录 → Extract 不剥离），gh.exe 落在 <dest>/bin/ 下。
// 经 GhRuntime.liftBin 提升后应位于版本目录根，且空的 bin/ 被清理。
func TestGhInstallLiftsBin(t *testing.T) {
	dir := t.TempDir()
	...
	// Extract 阶段：多顶层段不剥离，gh.exe 仍在 bin/ 下
	if _, err := os.Stat(filepath.Join(dest, "bin", "gh.exe")); err != nil {
		t.Fatalf("expected gh.exe under bin/ before lift: %v", err)
	}

	g := &GhRuntime{}
	if err := g.liftBin(dest); err != nil {
		t.Fatal(err)
	}
	// 提升后 gh.exe 位于版本根，bin/ 已移除，其余顶层内容保留
	for _, p := range []string{"gh.exe", "share/man/gh.1", "LICENSE"} {
		if _, err := os.Stat(filepath.Join(dest, filepath.FromSlash(p))); err != nil {
			t.Fatalf("expected %s after lift: %v", p, err)
		}
	}
	if _, err := os.Stat(filepath.Join(dest, "bin")); !os.IsNotExist(err) {
		t.Fatal("empty bin/ should be removed after lift")
	}
}
