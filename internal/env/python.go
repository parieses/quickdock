...
func (p *PythonRuntime) SupportedPlatforms() []string { return []string{"windows", "darwin"} }
...
func (p *PythonRuntime) ExeFor(version string) string {
	...
	// python-build-standalone 便携包：bin/python3
	return filepath.Join(p.versionDir(version), "bin", "python3")
}
