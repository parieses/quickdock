...
func (f *FFmpegRuntime) SupportedPlatforms() []string { return []string{"windows", "darwin"} }
...
func (f *FFmpegRuntime) ExeFor(version string) string {
	...
	// mac/linux：ffmpeg-static 为单文件二进制（bin/ffmpeg）
	return filepath.Join(f.versionDir(version), "bin", "ffmpeg")
}
