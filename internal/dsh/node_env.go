...
// installDir 现行 dsh 软件包安装目录（dataDir/dsh）。
// node 仅作运行底座（PATH 优先、缺失才自装便携），dsh 软件包（@deepseek-ai/dsh 的
// node_modules）由 QuickDock 独立管理在自含目录：npm install -g --prefix <此目录>。
// 这样 dsh 不寄生任何外部软件（如 PhpWebStudy）的 npm 全局，随外部 node 升级/清理而失效。
// 历史版本曾用裸 `npm install -g`（无 --prefix）装进 node 默认全局，此目录仅兜底兼容。
...
// 按优先级探测：QuickDock 自含目录 dataDir/dsh → PATH 官方全局 → 便携 node 的 npm 默认全局。
...
// 1 QuickDock 自含目录（dataDir/dsh，现行主安装，npm -g --prefix 落点）→ 2 官方全局（PATH）
// → 3 npm 默认全局 bin → 4 npm root -g → 5 npx 临时缓存 → 6 便携 node 的 npm 全局（兜底）。
// dataDir/dsh 置顶：dsh 软件包由 QuickDock 独立管理，启动优先用这份，不命中外部全局副本。
func (m *NodeEnvManager) dshMainJSCandidates() []string {
	var cands []string
	// 自含目录 node_modules 下（npm -g --prefix <dir> 的标准落点）
	cands = append(cands,
...
func (m *NodeEnvManager) runNpmInstall(ctx context.Context, target string, onMsg func(string)) error {
	...
	// dsh 软件包独立安装到自含目录（dataDir/dsh，npm -g --prefix），不寄生外部 node 的
	// npm 全局。node 仅当运行底座：执行 npm 的 node 仍取 NodePath()（PATH 优先/缺失自装）。
	// 产物落在 <installDir>/node_modules/@deepseek-ai/dsh + <installDir> 根 shim。
	args := []string{npmCli, "install", "-g", "--prefix", m.installDir(), pkgArg}
	...
	env = append(env,
		...
		// 让 npm 的 bin shim 前缀也指向自含目录，子进程经 npm 拉起的 node 引用保持一致
		"npm_config_prefix="+m.installDir(),
	)
